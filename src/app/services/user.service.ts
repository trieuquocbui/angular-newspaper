...
getUserList(search: string, currentPageNumber: number, offset: number, sortBy: string, sortDir: string):Observable<PagenationModel<UserModel>> {
  const params: Params = {
    search : search,
  ...
  return this.http.get<PagenationModel<UserModel>>(`${baseUrl}/user/list`,{headers:reqHeader, params:queryParams});
}
